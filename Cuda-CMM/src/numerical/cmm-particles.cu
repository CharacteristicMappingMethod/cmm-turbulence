...
__global__ void Particle_advect(int Nb_particle, double dt, double *particles_pos, double *psi, int N, int NX, int NY, double h, int particles_time_integration_num){
	...
	switch (particles_time_integration_num) {
		...
		case 20: {
			...
			double k2[2] = {part_pos_old[0] + dt*u[2], part_pos_old[1] + dt*u[3]};
		...
		case 25: {
			...
			particles_pos[2 * i] = part_pos_old[0] +  0.5 * dt * (psi_y + psi_y_p);
			particles_pos[2 * i + 1] = part_pos_old[1] -  0.5 * dt * (psi_x + psi_x_p);
			break;
		}
		...
		case 35: {
	        ...
	        particles_pos[2 * i] = part_pos_old[0] +  dt * (k1_x + 4*k2_x + k3_x)/6;
	        particles_pos[2 * i + 1] = part_pos_old[1] + dt * (k1_y + 4*k2_y + k3_y)/6;
	        break;
		}
		// default: zero
...
__global__ void Particle_advect_inertia(int Nb_particle, double dt, double *particles_pos, double *particles_vel, double *psi, int N, int NX, int NY, double h, double tau_p, int particles_time_integration_num) {
	...
	switch (particles_time_integration_num) {
		...
		case 20: {
			...
			double k2[2] = {part_pos_old[0] + dt * kp1[0], part_pos_old[1] + dt * kp1[1]};
			device_hermite_interpolate_grad_2D(psi, k2, u, NX, NY, h, 1);
			...
			particles_pos[2*i+1] += dt/2.0 * (kp1[1] + kp2[1]);
		...
		case 30: {
			...
			double k2[2] = {part_pos_old[0] + dt/2.0 * kp1[0], part_pos_old[1] + dt/2.0 * kp1[1]};
			device_hermite_interpolate_grad_2D(psi, k2, u, NX, NY, h, 3);
			...
			double k3[2] = {part_pos_old[0] + dt*(-kp1[0]+2*kp2[0]), part_pos_old[1] + dt*(-kp1[1]+2*kp2[1])};
			device_hermite_interpolate_grad_2D(psi, k3, u, NX, NY, h, 1);
		...
		case 40: {
			...
			k2[1] = - (kp1[1] + dt/2.0*k1[1] - (0.3125 * u[1] + 0.9375 * u[3] + -0.3125 * u[5] + 0.0625 * u[7])) / tau_p;


			// kp3 = F_p(p_n + h/2 kp_2, v_n + h/2 kv_2, t_n+1/2) = v_n + h/2 kv_2
			double kp3[2] = {kp1[0] + dt/2.0 * k2[0], kp1[1] + dt/2.0 * k2[1]};

			// compute u(p_n + h/2 kp2, t_n+1/2)
			double k3[2] = {part_pos_old[0] + dt/2.0 * kp2[0], part_pos_old[1] + dt/2.0 * kp2[1]};
			device_hermite_interpolate_grad_2D(psi, k3, u, NX, NY, h, 4);
			...
			k3[1] = - (kp1[1] + dt/2.0*k2[1] - (0.3125 * u[1] + 0.9375 * u[3] + -0.3125 * u[5] + 0.0625 * u[7])) / tau_p;
			...
			double k4[2] = {part_pos_old[0] + dt*kp3[0], part_pos_old[1] + dt*kp3[1]};
			device_hermite_interpolate_grad_2D(psi, k4, u, NX, NY, h, 1);
		...
		case 25: {
		    ...
		    particles_pos[2 * i] = part_pos_old[0] + dt * particles_vel[2 * i];
		    ...
		    particles_pos[2 * i + 1] = part_pos_old[1]  + dt * particles_vel[2 * i + 1];
			break;
		}
