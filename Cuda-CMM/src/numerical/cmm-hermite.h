...
// cubic functions with their first derivatives, functions 1-2 are represented
#define H_f1_3(x) (1 - 3*(x)*(x) + 2*(x)*(x)*(x))
#define H_f1x_3(x) (- 6 + 6*(x)) * (x)
#define H_f2_3(x) ((x)*(1 - (x))*(1 - (x)))
#define H_f2x_3(x) (1 + 3*(x)*(x) - 4*(x))
// quintic functions with their first derivatives, functions 4-6 are represented
#define H_f1_5(x) (10 - 15*(x) + 6*(x)*(x)) * (x)*(x)*(x)
#define H_f2_5(x) (-4 + 7*(x) - 3*(x)*(x)) * (x)*(x)*(x)
#define H_f3_5(x) (0.5 - x + 0.5*(x)*(x)) * (x)*(x)*(x)
